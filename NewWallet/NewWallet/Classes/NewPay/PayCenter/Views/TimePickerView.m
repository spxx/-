...
-(void)initUserInterface{
    ...
    [self.datePicker selectRow:self.yearStr.integerValue-2016 inComponent:0 animated:NO];
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    ...
    if ([_delegate respondsToSelector:@selector(getdatepicker:andMonth:)]) {
        ...
        [_delegate getdatepicker:[NSString stringWithFormat:@"%@",self.yearStr] andMonth:[NSString stringWithFormat:@"%@",self.monthStr]];
    }
}
...
-(NSMutableArray *)yearData{
    if (!_yearData) {
        ...
        for (int i = 2017; i<=[self.yearStr intValue]; i++) {
            NSString *str = [NSString stringWithFormat:@"%d",i];
