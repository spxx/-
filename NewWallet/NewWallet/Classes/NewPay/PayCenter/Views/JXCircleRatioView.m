...
- (void)drawRect:(CGRect)rect {
    ...
    if (_numArray.count>0) {
        ...
        for (int i = 0; i < self.numArray.count ; i ++) {
            ...
            [imageV sd_setImageWithURL:[NSURL URLWithString:_imagesArray[i]] placeholderImage:nil];
            if ((end-start)>=0.08) {
        ...
        if (_numArray) {
            UIBezierPath *arcPath = [UIBezierPath bezierPathWithArcCenter:CGPointMake(self.frame.size.width / 2, self.frame.size.height / 2) radius:_circleRadius startAngle:0 endAngle:M_PI * 2 clockwise:YES];
            [UIColorFromRGB(0xdbdbdb) set];
            [arcPath fill];
            [arcPath stroke];
        }
    }
...
-(void)refeshCostL:(NSString *)money{
    UILabel *costTitle = [UILabel new];
    costTitle.viewSize = CGSizeMake(100, 15);
    [costTitle align:ViewAlignmentCenter relativeToPoint:CGPointMake(self.viewWidth/2, self.viewHeight/2)];
    costTitle.text = @"总支出";
    costTitle.textColor = UIColorFromRGB(0x494d4f);
    costTitle.font = [UIFont systemFontOfSize:15];
    [costTitle sizeToFit];
    costTitle.viewHeight = 15;
    [costTitle align:ViewAlignmentCenter relativeToPoint:CGPointMake(self.viewWidth/2, self.viewHeight/2-6)];
    [self addSubview:costTitle];
    
    _costLabel = [UILabel new];
    _costLabel.viewSize = costTitle.viewSize;
    [_costLabel align:ViewAlignmentTopCenter relativeToPoint:CGPointMake(self.viewWidth/2, costTitle.viewBottomEdge+3)];
    _costLabel.textColor = UIColorFromRGB(0x494d4f);
    _costLabel.font = [UIFont systemFontOfSize:15];
    _costLabel.text = money;
    [_costLabel sizeToFit];
    [_costLabel align:ViewAlignmentTopCenter relativeToPoint:CGPointMake(self.viewWidth/2, costTitle.viewBottomEdge+3)];
    _costLabel.viewHeight = 15;
