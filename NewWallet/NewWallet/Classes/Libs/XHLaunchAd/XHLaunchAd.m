...
+(void)showWithAdFrame:(CGRect)frame setAdImage:(setAdImageBlock)setAdImage showFinish:(showFinishBlock)showFinish
{
    ...
    BNNavigationController *nav = [[BNNavigationController alloc]initWithRootViewController:AdVC];
...
-(UIButton *)skipButton
{
    if(_skipButton == nil)
    {
        ...
        _skipButton.frame = CGRectMake([UIScreen mainScreen].bounds.size.width-110,30, 100, 30);
...
//        UIView *roundGrayView = [[UIView alloc]initWithFrame:CGRectMake(0, 2, 26, 26)];
//        roundGrayView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.4];
//        roundGrayView.layer.cornerRadius = 13;
//        roundGrayView.layer.masksToBounds = YES;
//        roundGrayView.userInteractionEnabled = NO;
//        [_skipButton addSubview:roundGrayView];
...
-(void)skipButtonTitleWithDuration:(NSInteger)duration{
    ...
    switch (_skipType) {
        ...
        case SkipTypeTimeText:
            ...
            [_skipButton setTitle:[NSString stringWithFormat:@"  %ld    跳过广告",duration] forState:UIControlStateNormal];
...
+ (void)cancelTimer
{
    BNNavigationController *navVC = (BNNavigationController *)[[UIApplication sharedApplication].delegate window].rootViewController;
    XHLaunchAd *AdVC2;
    for (UIViewController *vc in navVC.viewControllers) {
        if ([vc isKindOfClass:[XHLaunchAd class]]) {
            AdVC2 = (XHLaunchAd *)vc;
        }
    }
    if (AdVC2) {
        dispatch_source_cancel(AdVC2.noDataTimer);
        [AdVC2 remove];
    }
