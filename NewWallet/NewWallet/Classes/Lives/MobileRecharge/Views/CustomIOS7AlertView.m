...
{
    BNLog(@"Button Clicked! %d, %d", (int)buttonIndex, (int)[alertView tag]);
    [self close];
}
...
- (UIView *)createContainerView
{
    if (containerView == NULL) {
        containerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 270, 150)];
    }
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    for (int i=0; i<[buttonTitles count]; i++) {
        ...
        if (([buttonTitles count] >= 2) && (i < [buttonTitles count] - 1)) {
            UIView *line = [[UIView alloc]initWithFrame:CGRectMake(closeButton.frame.origin.x + closeButton.frame.size.width, closeButton.frame.origin.y, 0.5, closeButton.frame.size.height)];
            line.backgroundColor = [UIColor colorWithRed:198.0/255.0 green:198.0/255.0 blue:198.0/255.0 alpha:1.0f];;
            [container insertSubview:line aboveSubview:closeButton];
        }
